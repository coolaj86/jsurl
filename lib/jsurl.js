...
module.exports.curl = function (program, fn) {
  ...
  dns.resolve4(program.location.hostname, function (err4, addr4) {
    dns.resolve6(program.location.hostname, function (err6, addr6) {
      ...
      if (program.ipv4 && !addr4) {
        console.info('jsurl: (4) Could not resolve host: aj.daplie.me');
        fn(err4);
        return;
      }
      if (program.ipv6 && !addr6) {
        console.info('jsurl: (6) Could not resolve host: aj.daplie.me');
        fn(err6);
        return;
      }
